  ...
  var GesturePasswd = function (element, options) {
    ...
    this.initDraw = function () {
      ...
      for (var j = 0; j < 3; j++) {
        for (var i = 0; i < 3; i++) {
          ...
          this.$ctx.moveTo(
            this.o / 2 + this.rr * 2 + i * (this.o + 2 * this.rr),
            this.o / 2 + this.rr + j * (this.o + 2 * this.rr)
          );
          //使用画弧方法,ctx.arc(100,75,50,0,2*Math.PI)
          this.$ctx.arc(
            this.o / 2 + this.rr + i * (this.o + 2 * this.rr),
            this.o / 2 + this.rr + j * (this.o + 2 * this.rr),
            this.rr,
            0,
            2 * Math.PI
      ...
      this.initImg = this.$ctx.getImageData(
        0,
        0,
        this.options.width,
        this.options.height
    ...
    this.isIn = function (x, y) {
      for (var p in that.pList) {
        ...
        if (
          Math.pow(x - that.pList[p]["x"], 2) +
            Math.pow(y - that.pList[p]["y"], 2) <
          Math.pow(this.rr, 2)
        ) {
          return that.pList[p];
        }
      ...
      for (var p in that.sList) {
        if (e.data.that.pointInList(that.sList[p], e.data.that.pList)) {
          ...
          e.data.that.result =
            e.data.that.result +
            e.data.that
              .pointInList(that.sList[p], e.data.that.pList)
              .toString();
        }
      }
      //手动触发"hasPasswd"事件
